...
interface LoginFormProps {
  // onLogin: (user: any) => void; // Bu prop artık doğrudan yönlendirme yapacağı için AuthPage'den gelmeyecek
  // onSwitchToRegister: () => void; // Bu prop artık doğrudan yönlendirme yapacağı için AuthPage'den gelmeyecek
}

export const LoginForm: React.FC<LoginFormProps> = () => { // Props'ları kaldırıyoruz
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // onLogin(user); // Artık doğrudan yönlendirme yapıyoruz
      console.log('Kullanıcı giriş yaptı:', user);
      alert('Giriş başarılı! Konsolu kontrol edin.');
      navigate('/dashboard'); // Başarılı giriş sonrası yönlendirilecek sayfa
    } catch (err) {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-blue-50 to-indigo-100 px-4">
      <div className="max-w-md w-full space-y-8 p-8 bg-white rounded-lg shadow-xl"> {/* Eklenen stiller */}
        ...
        <form className="mt-8 space-y-6" onSubmit={handleSubmit}>
          ...
          <div className="text-center">
            <button
              type="button"
              onClick={() => navigate('/register')} // Doğrudan /register yoluna yönlendirme
